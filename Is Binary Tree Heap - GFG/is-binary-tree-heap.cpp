...
class Solution {
  public:
    ...
    bool mo(struct Node* root)
    {
        ...
        {
            return mo(root->left) && mo(root->right) && root->left->data<root->data && root->right->data<root->data;
        }
    }
    bool cbt(struct Node* root,int index,int total)
    {
    ...
    bool isHeap(struct Node* tree) {
        ...
        if(cbt(tree,0,total) && mo(tree)) return true;
